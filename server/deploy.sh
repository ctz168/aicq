...
#  特性:
#    - 自动安装 Node.js 20.x + PM2 + Nginx + ClickHouse
#    - 自动编译 TypeScript 源码
#    - 自动生成 .env 配置文件
#    - 自动配置 Nginx 反向代理 + WebSocket + SSL
#    - 支持 Let's Encrypt 自动申请证书
#    - 支持自定义端口、域名、好友上限、ClickHouse 参数等
#    - 支持从已有源码目录部署（无需重新克隆）
#    - 内置健康检查和部署验证（含 ClickHouse 连通性）
#
#  用法:
#    chmod +x deploy.sh
#    sudo ./deploy.sh [选项]
...
#    --ch-url=URL             ClickHouse 地址 (默认: http://localhost:8123)
#    --ch-user=USER           ClickHouse 用户 (默认: default)
#    --ch-password=PASS       ClickHouse 密码 (默认: 自动生成)
#    --ch-database=DB         ClickHouse 数据库 (默认: aicq)
#    --no-color               禁用彩色输出
#    --help                   显示帮助信息
#
#  示例:
...
#    sudo ./deploy.sh --ch-password=MyStrongPassword123
#
#  最简用法:
#    sudo ./deploy.sh
# ============================================================================
...
echo ""
log_line
echo -e "${BOLD}${CYAN}          AICQ Server 独立一键部署脚本 v3.0${NC}"
log_line
...
echo -e "  安装目录:    ${APP_DIR}"
echo -e "  ClickHouse:  ${CH_URL} (${CH_DATABASE})"
...
next_step "安装系统依赖"
apt-get update -qq
apt-get install -y -qq curl wget git unzip software-properties-common lsb-release apt-transport-https ca-certificates gnupg > /dev/null 2>&1
log_ok "系统基础依赖安装完成"
...
# ─── 步骤 4: 安装 ClickHouse ─────────────────────────────────────────────
next_step "安装 ClickHouse 数据库"
ACTUAL_CH_PASSWORD=$(generate_ch_password)

if command -v clickhouse-client &>/dev/null; then
  CH_VERSION=$(clickhouse-client --version 2>/dev/null | head -1 || echo "unknown")
  log_ok "ClickHouse 已安装 (${CH_VERSION})"
else
  log_info "  添加 ClickHouse APT 仓库..."
  # 添加 ClickHouse 官方 GPG key 和仓库
  if ! apt-cache policy clickhouse-server 2>/dev/null | grep -q "clickhouse.com"; then
    # ClickHouse 官方安装方式 (Debian/Ubuntu)
    DEB_ARCH=$(dpkg --print-architecture)
    DEB_VERSION=$(lsb_release -cs 2>/dev/null || echo "jammy")
    ...
    # 使用官方安装脚本
    log_info "  正在下载并安装 ClickHouse..."
    curl -fsSL https://packages.clickhouse.com/deb/KEY.gpg | gpg --dearmor -o /usr/share/keyrings/clickhouse.gpg 2>/dev/null || true

    echo "deb [signed-by=/usr/share/keyrings/clickhouse.gpg arch=${DEB_ARCH}] https://packages.clickhouse.com/deb stable main" \
    ...
    DEBIAN_FRONTEND=noninteractive apt-get install -y -qq \
      ...
      clickhouse-common-static 2>&1 | tail -5 || {
        log_warn "APT 安装 ClickHouse 失败，尝试官方安装方式..."
        # 备选：直接使用官方安装脚本
        curl -fsSL https://clickhouse.com/ | sh 2>/dev/null || true
        if command -v clickhouse &>/dev/null; then
          clickhouse install --noninteractive 2>/dev/null || true
        fi
      }
  else
    DEBIAN_FRONTEND=noninteractive apt-get install -y -qq clickhouse-server clickhouse-client 2>&1 | tail -3
  fi

  if command -v clickhouse-client &>/dev/null; then
    log_ok "ClickHouse 安装完成"
  else
    log_error "ClickHouse 安装失败！请手动安装: https://clickhouse.com/docs/en/install"
    exit 1
  fi
fi

# 确保 ClickHouse 服务运行
log_info "  确保 ClickHouse 服务运行中..."
if systemctl is-active --quiet clickhouse-server 2>/dev/null; then
  log_ok "ClickHouse 服务已运行"
elif pgrep -x clickhouse-server &>/dev/null; then
  log_ok "ClickHouse 进程已运行"
else
  log_info "  启动 ClickHouse 服务..."
  # 设置密码
  if [ -n "$ACTUAL_CH_PASSWORD" ]; then
    clickhouse-server --password="${ACTUAL_CH_PASSWORD}" &
    CLICKHOUSE_PID=$!
    sleep 3
    if ! kill -0 "$CLICKHOUSE_PID" 2>/dev/null; then
      # 使用 systemd 启动
      systemctl start clickhouse-server 2>/dev/null || service clickhouse-server start 2>/dev/null || true
      sleep 3
    fi
  else
    systemctl start clickhouse-server 2>/dev/null || service clickhouse-server start 2>/dev/null || true
...
if curl -sf "${CH_URL}/ping" > /dev/null 2>&1; then
  log_ok "ClickHouse 连接成功: ${CH_URL}"
else
  log_warn "ClickHouse HTTP 接口未就绪，将在服务启动后重试"
  log_info "  如果启动失败，请检查: systemctl status clickhouse-server"
fi

# 创建数据库
if command -v clickhouse-client &>/dev/null; then
  log_info "  创建数据库: ${CH_DATABASE}"
  if [ -n "$ACTUAL_CH_PASSWORD" ]; then
    clickhouse-client --password="${ACTUAL_CH_PASSWORD}" -q "CREATE DATABASE IF NOT EXISTS ${CH_DATABASE}" 2>/dev/null || true
  else
    clickhouse-client -q "CREATE DATABASE IF NOT EXISTS ${CH_DATABASE}" 2>/dev/null || true
  fi
  log_ok "数据库 ${CH_DATABASE} 就绪"
fi

# ─── 步骤 5: 获取/更新源码 ───────────────────────────────────────────────
next_step "获取 AICQ 源码"
if [ -n "$SOURCE_DIR" ] && [ -d "${SOURCE_DIR}/server" ]; then
...
# ─── 步骤 6: 安装 npm 依赖 ──────────────────────────────────────────────
next_step "安装 npm 依赖"
cd "${APP_DIR}/shared/crypto"
log_info "  安装 shared/crypto..."
...
# ClickHouse 密码
CLICKHOUSE_PASSWORD=${ACTUAL_CH_PASSWORD}
...
log_ok "配置文件已生成: ${APP_DIR}/server/.env"
log_warn "  JWT_SECRET: ${ACTUAL_JWT_SECRET:0:16}... (请妥善保管)"
log_warn "  CH_PASSWORD: ${ACTUAL_CH_PASSWORD:0:8}... (请妥善保管)"
...
# 等待服务就绪（含 ClickHouse 连接，需要更长时间）
log_info "  等待服务启动 (包含 ClickHouse 初始化，可能需要 30-60 秒)..."
if wait_for_service "http://127.0.0.1:${PORT}/health" 60 "AICQ Server"; then
  HEALTH=$(curl -s "http://127.0.0.1:${PORT}/health" 2>/dev/null || echo '{"status":"unknown"}')
  log_ok "健康检查通过: ${HEALTH}"

  # 验证 ClickHouse 存储状态
  if echo "$HEALTH" | grep -q '"storage":"clickhouse"'; then
    log_ok "ClickHouse 持久化存储已启用"
  else
    log_warn "ClickHouse 存储未确认，请检查日志: pm2 logs aicq-server --lines 50"
  fi
else
  ...
  echo ""
  echo "  常见问题排查:"
  echo "  1. ClickHouse 未运行: systemctl status clickhouse-server"
  echo "  2. ClickHouse 连接失败: curl ${CH_URL}/ping"
  echo "  3. 检查 .env 中的 CLICKHOUSE_* 配置"
  exit 1
fi
...
if command -v ufw &>/dev/null; then
  ...
  # ClickHouse 仅允许本地访问
  ufw deny 8123/tcp   comment 'ClickHouse-HTTP-blocked' 2>/dev/null || true
  ufw deny 9000/tcp   comment 'ClickHouse-TCP-blocked'   2>/dev/null || true
  ufw --force enable  2>/dev/null || true
  log_ok "UFW 防火墙已配置 (22, 80, 443 开放; ClickHouse 端口已屏蔽外部访问)"
else
...
if pm2 pid aicq-server &>/dev/null; then
  log_ok "  [1/6] PM2 进程运行中"
else
  log_error " [1/6] PM2 进程未运行"
  ERRORS=$((ERRORS + 1))
fi

# 检查 ClickHouse
if curl -sf "${CH_URL}/ping" > /dev/null 2>&1; then
  log_ok "  [2/6] ClickHouse 连接正常"
else
  log_error " [2/6] ClickHouse 连接失败"
  ERRORS=$((ERRORS + 1))
fi
...
if curl -sf "http://127.0.0.1:${PORT}/health" > /dev/null 2>&1; then
  ...
  log_error " [3/6] 本地健康检查失败"
  ERRORS=$((ERRORS + 1))
fi

# 检查存储状态
HEALTH_JSON=$(curl -s "http://127.0.0.1:${PORT}/health" 2>/dev/null || echo '{}')
if echo "$HEALTH_JSON" | grep -q '"storage":"clickhouse"'; then
  log_ok "  [4/6] ClickHouse 持久化已启用"
else
  log_warn " [4/6] ClickHouse 持久化状态未确认"
fi
...
if $SKIP_NGINX; then
  ...
  if systemctl is-active nginx &>/dev/null; then
    log_ok "  [5/6] Nginx 运行中"
  else
    log_error " [5/6] Nginx 未运行"
    ERRORS=$((ERRORS + 1))
  fi
fi
...
echo -e "    配置文件:    ${APP_DIR}/server/.env"
echo -e "    应用目录:    ${APP_DIR}"
...
echo ""
echo -e "  ${BOLD}ClickHouse 管理:${NC}"
echo -e "    控制台:      clickhouse-client --password=${ACTUAL_CH_PASSWORD:0:8}..."
echo -e "    数据库:      ${CH_DATABASE}"
echo -e "    HTTP 接口:   ${CH_URL}"
echo -e "    查看表:      clickhouse-client --password=xxx -q 'SHOW TABLES FROM ${CH_DATABASE}'"
echo -e "    数据大小:    clickhouse-client --password=xxx -q 'SELECT formatReadableSize(sum(bytes)) FROM system.parts WHERE database = \"${CH_DATABASE}\"'"
echo ""
if ! $SKIP_SSL && [ -z "$SSL_EMAIL" ]; then
  ...
  echo ""
  echo -e "    2. 配置 ClickHouse 数据备份:"
  echo -e "       clickhouse-client -q \"SYSTEM BACKUP ${CH_DATABASE}\""
  echo ""
fi
echo -e "  ${BOLD}快速命令:${NC}"
echo -e "    查看状态:    pm2 status"
echo -e "    查看日志:    pm2 logs aicq-server --lines 100"
echo -e "    更新部署:    cd ${APP_DIR} && git pull && cd shared/crypto && npm run build && cd ../../server && npm run build && pm2 restart aicq-server"
echo -e "    CH 备份:     clickhouse-client --password=xxx -q \"BACKUP TABLE ${CH_DATABASE}.* TO Disk('backups', '${CH_DATABASE}_backup')\""
echo ""
log_line
echo ""
